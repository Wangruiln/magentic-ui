...
class WebSurfer(BaseChatAgent, Component[WebSurferConfig]):
    ...
    async def lazy_init(
        ...
        self.flag = False  # Reset the flag for lazy init
                ...
                if not need_execute_tool:
                    ...
                    for action in response:
                        ...
                        if tool_call_name == "stop_action":
                            ...
                            yield Response(
                                chat_message=TextMessage(
                                    content=emited_responses[-1]+"[DONE]",
                                    source=self.name,
                                    models_usage=final_usage,
                                ),
                            )
                        ...
                        if tool_call_name == "answer_question":
                            action_result = action_result + "[DONE]"
    ...
    async def _execute_tool_deep_web_search(self, args: Dict[str, Any]) -> AsyncGenerator[Response, None]:
        ...
        if reset_prior_metadata:
            self._prior_metadata_hash = None
        # 新增：支持收集模式，如果args中有'collect'参数（默认True）
        collected_summaries = []
        ...
        if self.only_url:
            ...
            for elem_id in related_elements[:5]:  # 限制最多5个
                ...
                if new_page_tentative:
                    ...
                    collected_summaries.append(sub_summary)
                    description = f"[Sub-page] markdown for link:{new_page_tentative.url}: {sub_summary}"
                    yield Response(
                        chat_message=TextMessage(
                            content=description,
                            source=self.name,
            ...
            action_description = f"[Summary] Collected and summarized from {len(collected_summaries)} links: {overall_summary}"
            yield Response(
                chat_message=TextMessage(
    ...
    async def _execute_tool_visit_url_and_page_summary(self, url: str, question: Optional[str] = None,
        ...
        # If the argument contains a space, treat it as a search query
            ...
            error_message = f"Invalid URL: {url}. Please provide a valid URL."
